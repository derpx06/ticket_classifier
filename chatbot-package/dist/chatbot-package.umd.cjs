...
`,mt=()=>{if(document.getElementById(dt))return;let e=document.createElement(`style`);e.id=dt,e.textContent=pt,document.head.appendChild(e)},ht=(e,t)=>{let n=document.createElement(`div`);return n.className=`chatbot-bubble ${t}`,n.textContent=e,n},gt=(e,t,n)=>{let r=document.createElement(`button`);return r.type=`button`,r.innerHTML=`<i data-lucide="${e}" aria-hidden="true"></i>`,r.setAttribute(`aria-label`,t),n&&r.addEventListener(`click`,n),r},_t=()=>{v({icons:{Bot:ee,MessageCircle:h,SendHorizontal:g,UserRound:_,X:te,CheckCircle:m,ArrowLeft:p}})},vt=e=>e.replace(/\/+$/,``),yt=e=>e.startsWith(`/`)?e:`/${e}`,bt=e=>e.replace(/\/api\/?$/i,``),xt=e=>e&&typeof e==`object`&&`data`in e?e.data:e;e.createChatbotWidget=(e={})=>{if(typeof window>`u`||typeof document>`u`)throw Error(`chatbot-package can only run in a browser environment.`);mt();let t={...ft,...e},n=document.createElement(`div`);n.className=`chatbot-widget-root ${t.position===`bottom-left`?`left`:`right`}`,n.style.setProperty(`--chatbot-primary`,t.primaryColor),n.style.zIndex=String(t.zIndex);let r=document.createElement(`button`);r.type=`button`,r.className=`chatbot-launcher`,r.setAttribute(`aria-label`,`Open chatbot`),r.setAttribute(`aria-expanded`,`false`),r.innerHTML=`<i data-lucide="message-circle" aria-hidden="true"></i>`;let i=document.createElement(`section`);i.className=`chatbot-panel`,i.setAttribute(`role`,`dialog`),i.setAttribute(`aria-label`,t.title);let a=document.createElement(`header`);a.className=`chatbot-header`;let o=document.createElement(`div`);o.className=`chatbot-header-left`;let s=document.createElement(`div`);s.className=`chatbot-avatar`,s.innerHTML=`<i data-lucide="bot" aria-hidden="true"></i>`;let c=document.createElement(`div`);c.className=`chatbot-header-info`;let l=document.createElement(`h2`);l.textContent=t.title;let u=document.createElement(`div`);u.className=`chatbot-status`;let d=document.createElement(`span`);d.className=`chatbot-status-dot`;let f=document.createElement(`span`);f.textContent=t.subtitle,u.append(d,f),c.append(l,u),o.append(s,c);let p=document.createElement(`div`);p.className=`chatbot-controls`;let ee=gt(`x`,`Close chat`,()=>{n.classList.remove(`open`),r.setAttribute(`aria-expanded`,`false`)});p.append(ee),a.append(o,p);let m=document.createElement(`div`);m.className=`chatbot-body`;let h=document.createElement(`section`);h.className=`chatbot-intro`;let g=document.createElement(`div`);g.className=`chatbot-intro-icon`,g.textContent=`?`;let _=document.createElement(`h3`);_.textContent=`Hi there!`;let te=document.createElement(`p`);te.textContent=`I am your AI support assistant. Ask me anything.`;let v=document.createElement(`p`);v.className=`chatbot-human-note`,v.textContent=`If I cannot help, you can connect with our human support team.`,h.append(g,_,te,v);let y=document.createElement(`div`);y.className=`chatbot-messages`,y.setAttribute(`aria-live`,`polite`),y.appendChild(ht(t.welcomeMessage,`bot`)),m.append(h,y);let b=document.createElement(`div`);b.className=`chatbot-footer`;let x=document.createElement(`form`);x.className=`chatbot-input-row`;let S=document.createElement(`input`);S.type=`text`,S.placeholder=t.placeholder;let C=document.createElement(`button`);C.type=`submit`,C.setAttribute(`aria-label`,`Send message`),C.innerHTML=`<i data-lucide="send-horizontal" aria-hidden="true"></i>`,x.append(S,C);let w=document.createElement(`button`);w.type=`button`,w.className=`chatbot-human-button`;let T=`<i data-lucide="user-round" aria-hidden="true"></i><span>Talk to a real human</span>`;w.innerHTML=T;let E=document.createElement(`p`);E.className=`chatbot-powered`,E.innerHTML=`Powered by <strong>AI assistant</strong>`,b.append(x,w,E);let D=document.createElement(`div`);D.className=`chatbot-human-container`;let O=document.createElement(`div`);O.className=`chatbot-human-header`;let ne=document.createElement(`h3`);ne.textContent=`Contact Support`;let re=document.createElement(`p`);re.textContent=`Please provide your details and we will get back to you shortly.`,O.append(ne,re);let k=document.createElement(`form`);k.className=`chatbot-human-form`;let ie=(e,t)=>{let n=document.createElement(`div`);n.className=`chatbot-form-group`;let r=document.createElement(`label`);return r.textContent=e,n.append(r,t),n},A=document.createElement(`input`);A.type=`text`,A.placeholder=`John Doe`,A.required=!0;let j=document.createElement(`input`);j.type=`email`,j.placeholder=`john@example.com`,j.required=!0;let M=document.createElement(`textarea`);M.placeholder=`How can we help you?`,M.required=!0;let N=document.createElement(`div`);N.className=`chatbot-form-actions`;let P=document.createElement(`button`);P.type=`button`,P.className=`chatbot-btn-secondary`,P.innerHTML=`<i data-lucide="arrow-left" aria-hidden="true" style="width: 16px; height: 16px;"></i> Back`;let F=document.createElement(`button`);F.type=`submit`,F.className=`chatbot-btn-primary`,F.textContent=`Send Message`,N.append(P,F),k.append(ie(`Name`,A),ie(`Email`,j),ie(`Description`,M),N);let I=document.createElement(`div`);I.className=`chatbot-human-success`;let ae=document.createElement(`div`);ae.className=`chatbot-success-icon`,ae.innerHTML=`<i data-lucide="check-circle" aria-hidden="true"></i>`;let L=document.createElement(`h3`);L.textContent=`Message Sent!`;let R=document.createElement(`p`);R.textContent=`Our support team will reach out to you via email shortly.`;let z=document.createElement(`button`);z.type=`button`,z.className=`chatbot-btn-primary`,z.textContent=`Back to Chat`,I.append(ae,L,R,z),D.append(O,k,I),i.append(a,m,b,D),n.append(i,r),document.body.appendChild(n),_t();let B=!1,V=!1,H=!1,U=null,W=null,oe=null,se=e.aiSupport?.sessionId||(typeof crypto<`u`&&typeof crypto.randomUUID==`function`?crypto.randomUUID():`widget-${Date.now()}-${Math.random().toString(36).slice(2,10)}`),G=e=>{e.trim()&&(m.classList.add(`has-messages`),y.appendChild(ht(e.trim(),`bot`)),m.scrollTop=m.scrollHeight)},K=e=>{V||(B=e,n.classList.toggle(`open`,B),r.setAttribute(`aria-expanded`,String(B)),B&&window.setTimeout(()=>S.focus(),0))},q=async n=>{if(H&&U){U.emit(`widget:message`,{text:n});return}if(e.onUserMessage){let t=await e.onUserMessage(n);typeof t==`string`&&t.trim()&&G(t);return}if(e.aiSupport){try{let t=vt(e.aiSupport.apiBaseUrl),r=yt(e.aiSupport.chatPath||`/rag/chat`),i=await fetch(`${t}${r}`,{method:`POST`,headers:{"Content-Type":`application/json`,"x-api-key":e.aiSupport.apiKey},body:JSON.stringify({query:n,sessionId:se})});if(!i.ok)throw Error(`Unable to fetch chatbot response right now.`);let a=xt(await i.json());G(typeof a?.answer==`string`&&a.answer||typeof a?.response==`string`&&a.response||typeof a?.message==`string`&&a.message||`I processed your question, but no answer text was returned.`)}catch(e){G(e instanceof Error?e.message:`Sorry, I am having trouble connecting right now.`)}return}G(`Thanks! ${t.botName} received: "${n}"`)},ce=async n=>{if(!t.humanSupport){G(`Human support is not configured for this widget yet.`);return}let r=t.humanSupport.widgetKey||e.aiSupport?.apiKey;if(!r)throw Error(`Human support requires a widget key or aiSupport.apiKey.`);let i=vt(t.humanSupport.apiBaseUrl),a=await fetch(`${i}/widget/session`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({widgetKey:r,visitorName:n.name,visitorEmail:n.email,issue:n.issue})});if(!a.ok)throw Error(`Unable to connect to human support right now.`);let o=xt(await a.json());W=o.sessionId,oe=o.ticketId,H=!0;let s=ut(bt(i),{path:`/socket.io`,transports:[`websocket`],auth:{token:o.chatToken}});U=s,s.on(`connect`,()=>{f.textContent=`Connected with human support`}),s.on(`disconnect`,()=>{H&&(f.textContent=`Reconnecting to human support...`)}),s.on(`chat:message`,e=>{e.sessionId===W&&e.sender===`agent`&&typeof e.text==`string`&&G(e.text)}),s.on(`chat:error`,e=>{G(e.message||`Support connection error. Please try again.`)}),s.emit(`widget:request_human`,{name:n.name,email:n.email,issue:n.issue})},le=async e=>{let t=e.trim();!t||V||(m.classList.add(`has-messages`),y.appendChild(ht(t,`user`)),S.value=``,m.scrollTop=m.scrollHeight,await q(t))};return r.addEventListener(`click`,()=>{K(!B)}),x.addEventListener(`submit`,async e=>{e.preventDefault(),await le(S.value)}),w.addEventListener(`click`,async()=>{if(e.onTalkToHumanClick){w.disabled=!0,w.textContent=`Creating ticket...`;try{let t=await e.onTalkToHumanClick();typeof t==`string`&&t.trim()&&G(t)}catch(e){G(e instanceof Error?e.message:`Unable to create support ticket right now.`)}finally{w.disabled=!1,w.innerHTML=T,_t()}return}n.classList.add(`show-human-form`),n.classList.remove(`show-human-success`)}),P.addEventListener(`click`,()=>{n.classList.remove(`show-human-form`)}),k.addEventListener(`submit`,async e=>{e.preventDefault(),F.disabled=!0,F.textContent=`Connecting...`;try{await ce({name:A.value.trim(),email:j.value.trim(),issue:M.value.trim()}),n.classList.add(`show-human-success`),L.textContent=`Connected to Support`,R.textContent=`You can now continue chatting with a human support agent.`,F.textContent=`Send Message`}catch(e){G(e instanceof Error?e.message:`Unable to connect to support right now.`),F.textContent=`Send Message`}finally{F.disabled=!1}}),z.addEventListener(`click`,()=>{n.classList.remove(`show-human-form`),n.classList.remove(`show-human-success`),k.reset(),m.classList.add(`has-messages`),y.appendChild(ht(H&&oe?`You are now connected with our support team (ticket ${oe.slice(-6)}).`:`Your issue has been submitted. A human agent will contact you soon.`,`bot`)),m.scrollTop=m.scrollHeight}),{open:()=>K(!0),close:()=>K(!1),toggle:()=>K(!B),sendMessage:le,destroy:()=>{V||(V=!0,U&&=(U.close(),null),n.remove())}}}});