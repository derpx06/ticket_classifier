...
`,ht=()=>{if(document.getElementById(ft))return;let e=document.createElement(`style`);e.id=ft,e.textContent=mt,document.head.appendChild(e)},$=(e,t)=>{let n=document.createElement(`div`);return n.className=`chatbot-bubble ${t}`,n.textContent=e,n},gt=(e,t,n)=>{let r=document.createElement(`button`);return r.type=`button`,r.innerHTML=`<i data-lucide="${e}" aria-hidden="true"></i>`,r.setAttribute(`aria-label`,t),n&&r.addEventListener(`click`,n),r},_t=()=>{v({icons:{Bot:ee,MessageCircle:h,SendHorizontal:g,UserRound:_,X:te,CheckCircle:m,ArrowLeft:p}})},vt=e=>e.replace(/\/+$/,``),yt=e=>e.startsWith(`/`)?e:`/${e}`,bt=e=>e.replace(/\/api\/?$/i,``),xt=e=>e&&typeof e==`object`&&`data`in e?e.data:e;e.createChatbotWidget=(e={})=>{if(typeof window>`u`||typeof document>`u`)throw Error(`chatbot-package can only run in a browser environment.`);ht();let t={...pt,...e},n=document.createElement(`div`);n.className=`chatbot-widget-root ${t.position===`bottom-left`?`left`:`right`}`,n.style.setProperty(`--chatbot-primary`,t.primaryColor),n.style.zIndex=String(t.zIndex);let r=document.createElement(`button`);r.type=`button`,r.className=`chatbot-launcher`,r.setAttribute(`aria-label`,`Open chatbot`),r.setAttribute(`aria-expanded`,`false`),r.innerHTML=`<i data-lucide="message-circle" aria-hidden="true"></i>`;let i=document.createElement(`section`);i.className=`chatbot-panel`,i.setAttribute(`role`,`dialog`),i.setAttribute(`aria-label`,t.title);let a=document.createElement(`header`);a.className=`chatbot-header`;let o=document.createElement(`div`);o.className=`chatbot-header-left`;let s=document.createElement(`div`);s.className=`chatbot-avatar`,s.innerHTML=`<i data-lucide="bot" aria-hidden="true"></i>`;let c=document.createElement(`div`);c.className=`chatbot-header-info`;let l=document.createElement(`h2`);l.textContent=t.title;let u=document.createElement(`div`);u.className=`chatbot-status`;let d=document.createElement(`span`);d.className=`chatbot-status-dot`;let f=document.createElement(`span`);f.textContent=t.subtitle,u.append(d,f),c.append(l,u),o.append(s,c);let p=document.createElement(`div`);p.className=`chatbot-controls`;let ee=gt(`x`,`Close chat`,()=>{n.classList.remove(`open`),r.setAttribute(`aria-expanded`,`false`)});p.append(ee),a.append(o,p);let m=document.createElement(`div`);m.className=`chatbot-body`;let h=document.createElement(`section`);h.className=`chatbot-intro`;let g=document.createElement(`div`);g.className=`chatbot-intro-icon`,g.textContent=`?`;let _=document.createElement(`h3`);_.textContent=`Hi there!`;let te=document.createElement(`p`);te.textContent=`I am your AI support assistant. Ask me anything.`;let v=document.createElement(`p`);v.className=`chatbot-human-note`,v.textContent=`If I cannot help, you can connect with our human support team.`,h.append(g,_,te,v);let y=document.createElement(`div`);y.className=`chatbot-messages`,y.setAttribute(`aria-live`,`polite`),y.appendChild($(t.welcomeMessage,`bot`)),m.append(h,y);let b=document.createElement(`div`);b.className=`chatbot-footer`;let x=document.createElement(`form`);x.className=`chatbot-input-row`;let S=document.createElement(`input`);S.type=`text`,S.placeholder=t.placeholder;let C=document.createElement(`button`);C.type=`submit`,C.setAttribute(`aria-label`,`Send message`),C.innerHTML=`<i data-lucide="send-horizontal" aria-hidden="true"></i>`,x.append(S,C);let w=document.createElement(`button`);w.type=`button`,w.className=`chatbot-human-button`;let T=`<i data-lucide="user-round" aria-hidden="true"></i><span>Talk to a real human</span>`;w.innerHTML=T;let E=document.createElement(`p`);E.className=`chatbot-powered`,E.innerHTML=`Powered by <strong>AI assistant</strong>`,b.append(x,w,E);let D=document.createElement(`div`);D.className=`chatbot-human-container`;let O=document.createElement(`div`);O.className=`chatbot-human-header`;let ne=document.createElement(`h3`);ne.textContent=`Contact Support`;let re=document.createElement(`p`);re.textContent=`Please provide your details and we will get back to you shortly.`,O.append(ne,re);let k=document.createElement(`form`);k.className=`chatbot-human-form`;let ie=(e,t)=>{let n=document.createElement(`div`);n.className=`chatbot-form-group`;let r=document.createElement(`label`);return r.textContent=e,n.append(r,t),n},A=document.createElement(`input`);A.type=`text`,A.placeholder=`John Doe`,A.required=!0;let j=document.createElement(`input`);j.type=`email`,j.placeholder=`john@example.com`,j.required=!0;let M=document.createElement(`textarea`);M.placeholder=`How can we help you?`,M.required=!0;let N=document.createElement(`div`);N.className=`chatbot-form-actions`;let P=document.createElement(`button`);P.type=`button`,P.className=`chatbot-btn-secondary`,P.innerHTML=`<i data-lucide="arrow-left" aria-hidden="true" style="width: 16px; height: 16px;"></i> Back`;let F=document.createElement(`button`);F.type=`submit`,F.className=`chatbot-btn-primary`,F.textContent=`Send Message`,N.append(P,F),k.append(ie(`Name`,A),ie(`Email`,j),ie(`Description`,M),N);let I=document.createElement(`div`);I.className=`chatbot-human-success`;let L=document.createElement(`div`);L.className=`chatbot-success-icon`,L.innerHTML=`<i data-lucide="check-circle" aria-hidden="true"></i>`;let ae=document.createElement(`h3`);ae.textContent=`Message Sent!`;let R=document.createElement(`p`);R.textContent=`Our support team will reach out to you via email shortly.`;let z=document.createElement(`button`);z.type=`button`,z.className=`chatbot-btn-primary`,z.textContent=`Back to Chat`,I.append(L,ae,R,z),D.append(O,k,I),i.append(a,m,b,D),n.append(i,r),document.body.appendChild(n),_t();let B=!1,V=!1,H=!1,U=!1,W=null,oe=null,G=null,se=e.aiSupport?.sessionId||(typeof crypto<`u`&&typeof crypto.randomUUID==`function`?crypto.randomUUID():`widget-${Date.now()}-${Math.random().toString(36).slice(2,10)}`),K=e=>{e.trim()&&(m.classList.add(`has-messages`),y.appendChild($(e.trim(),`bot`)),m.scrollTop=m.scrollHeight)},q=e=>{V||(B=e,n.classList.toggle(`open`,B),r.setAttribute(`aria-expanded`,String(B)),B&&window.setTimeout(()=>S.focus(),0))},ce=async n=>{if(H&&W){W.emit(`widget:message`,{text:n});return}if(e.onUserMessage){let t=await e.onUserMessage(n);typeof t==`string`&&t.trim()&&K(t);return}if(e.aiSupport){try{let t=vt(e.aiSupport.apiBaseUrl),r=yt(e.aiSupport.chatPath||`/rag/chat`),i=await fetch(`${t}${r}`,{method:`POST`,headers:{"Content-Type":`application/json`,"x-api-key":e.aiSupport.apiKey},body:JSON.stringify({query:n,sessionId:se})});if(!i.ok)throw Error(`Unable to fetch chatbot response right now.`);let a=xt(await i.json());K(typeof a?.answer==`string`&&a.answer||typeof a?.response==`string`&&a.response||typeof a?.message==`string`&&a.message||`I processed your question, but no answer text was returned.`)}catch(e){K(e instanceof Error?e.message:`Sorry, I am having trouble connecting right now.`)}return}K(`Thanks! ${t.botName} received: "${n}"`)},le=async n=>{if(!t.humanSupport){K(`Human support is not configured for this widget yet.`);return}let r=t.humanSupport.widgetKey||e.aiSupport?.apiKey;if(!r)throw Error(`Human support requires a widget key or aiSupport.apiKey.`);let i=vt(t.humanSupport.apiBaseUrl),a=await fetch(`${i}/widget/session`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({widgetKey:r,visitorName:n.name,visitorEmail:n.email,issue:n.issue})});if(!a.ok)throw Error(`Unable to connect to human support right now.`);let o=xt(await a.json());oe=o.sessionId,G=o.ticketId,H=!0;let s=dt(bt(i),{path:`/socket.io`,transports:[`websocket`],auth:{token:o.chatToken}});W=s,s.on(`connect`,()=>{f.textContent=`Connecting to a human agent...`}),s.on(`disconnect`,()=>{H&&(f.textContent=`Reconnecting to human support...`)}),s.on(`chat:message`,e=>{e.sessionId===oe&&e.sender===`agent`&&typeof e.text==`string`&&K(e.text)}),s.on(`chat:ticket_status`,e=>{if(e.sessionId&&e.sessionId===oe||e.ticketId&&e.ticketId===G){if(e.status===`assigned`){U||K(`A human agent has accepted your chat. You are now connected.`),U=!0,f.textContent=`Connected with human support`;return}e.status===`pending`&&(f.textContent=`Connecting to a human agent...`)}}),s.on(`chat:error`,e=>{K(e.message||`Support connection error. Please try again.`)}),s.emit(`widget:request_human`,{name:n.name,email:n.email,issue:n.issue}),f.textContent=`Connecting to a human agent...`},ue=async e=>{let t=e.trim();!t||V||(m.classList.add(`has-messages`),y.appendChild($(t,`user`)),S.value=``,m.scrollTop=m.scrollHeight,await ce(t))};return r.addEventListener(`click`,()=>{q(!B)}),x.addEventListener(`submit`,async e=>{e.preventDefault(),await ue(S.value)}),w.addEventListener(`click`,async()=>{if(H){K(U?`You are already connected with a human agent in this chat.`:`Your request is already in queue. An agent will join shortly.`);return}if(e.onTalkToHumanClick){w.disabled=!0,w.textContent=`Creating ticket...`;try{let t=await e.onTalkToHumanClick();typeof t==`string`&&t.trim()&&K(t)}catch(e){K(e instanceof Error?e.message:`Unable to create support ticket right now.`)}finally{w.disabled=!1,w.innerHTML=T,_t()}return}n.classList.add(`show-human-form`),n.classList.remove(`show-human-success`)}),P.addEventListener(`click`,()=>{n.classList.remove(`show-human-form`)}),k.addEventListener(`submit`,async e=>{e.preventDefault(),F.disabled=!0,F.textContent=`Connecting...`;try{await le({name:A.value.trim(),email:j.value.trim(),issue:M.value.trim()}),n.classList.remove(`show-human-form`),n.classList.remove(`show-human-success`),k.reset(),m.classList.add(`has-messages`),y.appendChild($(G?`Ticket ${G.slice(-6)} created. Connecting you to a human agent now...`:`Your request is submitted. Connecting you to a human agent now...`,`bot`)),m.scrollTop=m.scrollHeight,w.disabled=!0,w.innerHTML=`<i data-lucide="check-circle" aria-hidden="true"></i><span>Human Support Requested</span>`,_t(),F.textContent=`Send Message`}catch(e){K(e instanceof Error?e.message:`Unable to connect to support right now.`),F.textContent=`Send Message`}finally{F.disabled=!1}}),z.addEventListener(`click`,()=>{n.classList.remove(`show-human-form`),n.classList.remove(`show-human-success`),k.reset(),m.classList.add(`has-messages`),y.appendChild($(H&&G?U?`You are now connected with our support team (ticket ${G.slice(-6)}).`:`Your ticket ${G.slice(-6)} is waiting for an available human agent.`:`Your issue has been submitted. A human agent will contact you soon.`,`bot`)),m.scrollTop=m.scrollHeight}),{open:()=>q(!0),close:()=>q(!1),toggle:()=>q(!B),sendMessage:ue,destroy:()=>{V||(V=!0,W&&=(W.close(),null),n.remove())}}}});